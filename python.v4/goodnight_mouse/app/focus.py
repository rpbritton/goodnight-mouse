...
class Focus(Subscription):
    ...
    def _register(self):
        ...
        pyatspi.Registry.registerEventListener(self._activate_handle, *self._ACTIVATE_EVENTS)
        pyatspi.Registry.registerEventListener(self._deactivate_handle, *self._DEACTIVATE_EVENTS)
        ImmediateTimeout.disable()

        self.active_window = self.force_get_active_window()

    def _deregister(self):
        pyatspi.Registry.deregisterEventListener(self._activate_handle, *self._ACTIVATE_EVENTS)
        pyatspi.Registry.deregisterEventListener(self._deactivate_handle, *self._DEACTIVATE_EVENTS)
    ...
    def force_get_active_window(cls):
        ...
        for application in desktop:
            if application.get_process_id() == active_pid:
                ...
                active_windows = collection.getMatches(cls._ACTIVE_WINDOW_MATCH_RULE, collection.SORT_ORDER_CANONICAL, 0, False)
